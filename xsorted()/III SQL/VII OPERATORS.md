...
```mysql
SELECT COUNT(column_name) # counts the rows in a given column from a specified table
FROM table_name; # returns a single value
```
...
SELECT 
	column_name
COUNT
	(column_name) # returns two rows ; first with each group of unique values
FROM 
...
AVG()
Calculates the average of all the Non-Null values belonging to a certain column of a table
...
HAVING
Refines the outputs from records that no not satisfy a certain condition - like WHERE, but applied to the GROUP BY block
```mysql
SELECT 
	column_name[s]
FROM
...
```mysql
SELECT * FROM employees
HAVING hire_date > 1999-01-01;
```
*These two blocks above both return the same results. HAVING's unique application is it can have aggregate functions for its conditions, where as WHERE cannot.*

Suppose you wanted to fine all the employees names of which appear more than 250 times in the employees table.
```mysql
SELECT first_name, COUNT(first_name) AS names_count FROM employees
GROUP BY first_name
WHERE 
	COUNT(first_name) > 250 # this is proper syntax but will return an error if executed
ORDER BY first_name;
```
...
HAVING
Allows us to set a condition AFTER the values that satisfy the prerequisite conditions are met. After retrieval, HAVING applies some additional condition to the retrieved data.
